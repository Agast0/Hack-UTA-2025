...
async def run_agent(request: AgentRequestModel):
    ...
    try:
        ...
        fastapi_request = FastAPIRequest(request.url, request.content_type, request.tester_user_id, request.test_cases)
        ...
        print(f"Tester user ID: {request.tester_user_id}")
